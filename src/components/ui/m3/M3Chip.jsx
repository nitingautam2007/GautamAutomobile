...
export default function M3Chip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'inline-flex items-center gap-2 h-8 px-3 rounded-m3-sm',
        'text-m3-label-lg font-medium',
        'transition-all duration-[var(--m3-duration-short4)] ease-[var(--m3-easing-standard)]',
        'cursor-pointer select-none',
        isFilter && selected
          ? 'bg-m3-secondary-container text-m3-on-secondary-container border border-m3-secondary-container'
          : isFilter
    ...
    >
      ...
      {trailingIcon && (
        <span
          className="w-4 h-4 flex-shrink-0 cursor-pointer hover:text-m3-on-surface"
          onClick={(e) => {
            e.stopPropagation();
            onTrailingClick?.();
          }}
